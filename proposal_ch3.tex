...
\begin{equation}
  ...
  \label{eq:deeponet}
\end{equation}
由Chen \& Chen（1995）万能逼近定理\cite{chen1995}，对任意连续紧算子$\calF$，存在有限$p$使得$\|\hat{\calF} - \calF\|_\infty < \varepsilon$。Lu等（2021）\cite{lu2021}在\textit{Nature Machine Intelligence}上已验证DeepONet在复杂非线性算子逼近中兼具高精度（误差$<1\%$）与高效推理（加速比$>10^4\times$）。训练时在对数域（$\log_{10}\BER$）计算损失，以处理BER的多数量级跨度。
...
\textbf{模型架构}：代理模型由两个串联模块构成。\textbf{数据流图编码器（GNN）}将译码器的硬件数据流图表示为有向图，使用GNN提取包含检查节点并行度、消息传递路径长度等关键拓扑特征。\textbf{性能回归MLP}将GNN提取的特征与工艺参数拼接后，经多层感知机分别预测面积、功耗、时延与吞吐量。训练数据使用Synopsys Design Compiler在28~nm、7~nm工艺库下对$>5000$个不同配置的LDPC/Polar译码器RTL进行综合生成。
...
由于代理模型的单次推理时间约为毫秒级，$10^4$次模型评估仅需约10秒，实现相较传统方法$>1000\times$的优化速度提升。图~\ref{fig:joint_opt}展示了优化迭代闭环。
