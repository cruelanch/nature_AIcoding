...
图~\ref{fig:e2e_pipeline}展示了E3A框架的总体架构。框架由五个核心模块串行协作，形成从自然语言需求到RTL代码的完整闭环。
...
\begin{tikzpicture}[
  ...
  mainbox/.style={rectangle,rounded corners=4pt,draw=natblue,line width=1.2pt,
    fill=lightblue!55,text width=11cm,align=center,inner sep=7pt,font=\small},
  predbox/.style={rectangle,rounded corners=4pt,draw=natblue!70,line width=1pt,
    fill=lightblue!30,text width=4.9cm,align=center,inner sep=6pt,font=\small},
  outbox/.style={rectangle,rounded corners=4pt,draw=natgreen,line width=1.2pt,
    fill=lightgreen!50,text width=11cm,align=center,inner sep=7pt,font=\small},
...
\node[predbox,below=1.2cm of constr,xshift=-3.0cm] (perf) {
  \textbf{算法性能代理模型}\\[3pt]DeepONet\\[2pt]
  \footnotesize $(\thetacode, \calT) \mapsto \BER(\SNR)$
};
\node[predbox,below=1.2cm of constr,xshift=3.0cm] (hw) {
  \textbf{硬件性能代理模型}\\[3pt]GNN + MLP\\[2pt]
  \footnotesize $(\thetahw,\mathrm{Tech}) \mapsto \{A,P,T_d\}$
};
\node[mainbox,below=1.2cm of perf,xshift=3.0cm] (optim) {
  \textbf{联合离散优化引擎}\\[3pt]
...
\draw[arr] (req) -- (llm);
\draw[arr] (llm) -- (constr);
\draw[arr] (constr.south) -- ++(0,-0.4) -| (perf.north);
\draw[arr] (constr.south) -- ++(0,-0.4) -| (hw.north);
\draw[arr] (perf.south) -| ([xshift=-1cm]optim.north);
\draw[arr] (hw.south) -| ([xshift=1cm]optim.north);
\draw[arr] (optim) -- (rtl);
\draw[arr] (rtl) -- (out);
\draw[darr] (optim.west) -- ++(-1.8,0) |- (constr.west)
  node[pos=0.85,above,font=\scriptsize,color=natblue!60]{迭代反馈};
\end{tikzpicture}
\caption{E3A端到端纠错码AI全自动设计框架总体架构。框架由五个核心模块构成，实现从自然语言需求到可综合RTL代码的全自动闭环设计流水线。}
\label{fig:e2e_pipeline}
\end{figure}

\subsection{通用信道抽象}\label{sec:channel_abstraction}

E3A框架的理论基础是\textbf{通用信道三元组}表示：
\begin{equation}
